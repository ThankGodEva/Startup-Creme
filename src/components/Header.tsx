            ...
            {showRoleMenu && (
              <div className="absolute right-0 mt-2 w-64 bg-white border border-slate-200 rounded-xl shadow-xl py-2 z-50">
                <div className="px-3 py-2 border-b border-slate-100">
                  <div className="text-xs font-semibold text-slate-900 truncate">{currentUser?.full_name}</div>
                  <div className="text-[11px] text-slate-500 truncate">{currentUser?.email}</div>
                  <div className="mt-1.5 flex items-center justify-between gap-1">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-mono uppercase font-semibold ${
                      currentUser?.role === 'admin' 
                        ? 'bg-amber-100 text-amber-900 border border-amber-300' 
                        : 'bg-slate-100 text-slate-700 border border-slate-200'
                    }`}>
                      {currentUser?.role === 'admin' && <ShieldCheck className="w-3 h-3 text-amber-700" />}
                    ...
                    <button
                      onClick={async () => {
                        ...
                        try {
                          await store.refreshCurrentUserRole();
                        } finally {
                          setTimeout(() => setIsSyncingRole(false), 500);
                        }
                      }}
                      disabled={isSyncingRole}
                      className="text-[10px] text-cyan-700 hover:text-cyan-900 font-medium flex items-center gap-1 px-1.5 py-0.5 rounded hover:bg-cyan-50 transition-colors disabled:opacity-50"
                      title="Fetch latest role directly from Supabase database"
                    >
                      <RefreshCw className={`w-2.5 h-2.5 ${isSyncingRole ? 'animate-spin' : ''}`} />
                      <span>{isSyncingRole ? 'Syncing...' : 'Sync Role'}</span>
                    </button>
                  </div>
                </div>
